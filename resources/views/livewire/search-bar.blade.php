<div id="search-bar" class="search-bar">
    <form class="d-flex search-bar__form" role="search" action="{{route('catalog')}}">
        ...
        <input wire:model.live="search" class="form-control search-bar__input" type="text" placeholder="Я ищу..."
    ...
    @if(sizeof($products) > 0 || sizeof($categories) > 0)
        <div class="dropdown-menu d-block py-0 mt-2 search-bar__menu">
            ...
            @foreach($categories as $category)
                <div class="px-3 py-1 border-bottom">
                    @if($category->id === 1)
                        <a href="{{ '/catalog?category=candles' }}"
                           class="link-underline link-underline-opacity-0 link-dark">
                            <div class="d-flex flex-column ml-3">
                                <span>Свечи</span>
                    ...
                    @elseif($category->id === 2)
                        <a href="{{ '/catalog?category=diffusor' }}"
                           class="link-underline link-underline-opacity-0 link-dark">
                            <div class="d-flex flex-column ml-3">
                                <span>Диффузоры</span>
...
<script>
    document.addEventListener('livewire:load', function () {
        ...
        searchButton.addEventListener('click', (e) => {
            e.preventDefault();
            Livewire.emit('search', searchForm.querySelector('input[name="search"]').value);
        });
    });
</script>
