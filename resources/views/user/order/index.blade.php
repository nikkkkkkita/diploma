            ...
            @foreach($data as $datum)
                <div class="p-card mb-5">
                    <div class="order-list__order-number">
                        <span>Заказ №{{ $datum['order']->id }} на {{ number_format( $datum['order']->total, 0, '', ' ')}} ₽ • </span>
                    </div>
                    <div class="contacts">
                        {{--                        поля в contact_information--}}
                        <p>{{$datum['order']->contactInformation->city}}</p>
                    </div>
                    <div class="order-wrapper">
                        @foreach($datum['products'] as $product)
                            <div class="order-card">
                                <div class="order-card__img card-img">
                                    <img src="{{ $product->getFirstMediaUrl('products') }}" alt="Фото продукта"
                                         class="product-img">
                                </div>
                                <div class="order-card__info">
                                    <p class="product__name">{{ $product->name }}</p>
                                    <span class="product__count">Количество • {{ $product->pivot->quantity }}</span>
                                    <p class="product__price">{{   number_format($product->pivot->price, 0, '', ' ')}}
                                        ₽</p>
                                    <p class="product__price">{{ $product->pivot->status }}</p>
                                </div>
